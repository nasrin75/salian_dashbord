...
export default function List() {
    ...
    const handlePermissionEditPage = useCallback(
        (permissionID) => () => {
            navigate(`/setting/permission/edit/${permissionID}`);
        },
        [navigate],
    );

    const handelDeletePermission = useCallback(
        (permission) => async () => {

            const confirmed = await dialogs.confirm(
                `آبا از حذف دسترسی  ${permission.name} مطمئنید?`,
                {
                    title: `حذف دسترسی?`,
                    severity: 'خطا',
                    okText: 'حذف',
